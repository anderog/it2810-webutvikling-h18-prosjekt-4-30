...
export default class MongoMemoryServer {
  ...
  async start(): Promise<boolean> {
    ...
    this.runningInstance = this._startUpInstance()
      .catch(err => {
        if (
          err.message === "Mongod shutting down" ||
          err === "Mongod shutting down"
        ) {
          this.debug(
            `Mongodb does not started. Trying to start on another port one more time...`
          );
  ...
  async _startUpInstance(): Promise<MongoInstanceDataT> {
    ...
    this.debug(
      `Starting MongoDB instance with following options: ${JSON.stringify(
        data
      )}`
    );
  ...
  async stop(): Promise<boolean> {
    const {
      ...
      port,
      tmpDir
    } = (await this.getInstanceData(): MongoInstanceDataT);

    this.debug(
      `Shutdown MongoDB server on port ${port} with pid ${instance.getPid() ||
        ""}`
    );
    await instance.kill();
  ...
  async getConnectionString(
    otherDbName: string | boolean = false
  ): Promise<string> {
