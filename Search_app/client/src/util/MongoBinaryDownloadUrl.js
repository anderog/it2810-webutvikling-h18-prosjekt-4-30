...
export default class MongoBinaryDownloadUrl {
  ...
  async getDownloadUrl(): Promise<string> {
    ...
    return `${process.env?.MONGOMS_DOWNLOAD_MIRROR ||
      "https://fastdl.mongodb.org"}/${this.platform}/${archive}`;
  }
  ...
  getLinuxOSVersionString(os: getos.Os): string {
    if (/ubuntu/i.test(os.dist)) {
      ...
      /rhel/i.test(os.dist) ||
      /centos/i.test(os.dist) ||
      /scientific/i.test(os.dist)
    ) {
    ...
    console.warn(
      `Unknown linux distro ${os.dist}, falling back to legacy MongoDB build`
    );
