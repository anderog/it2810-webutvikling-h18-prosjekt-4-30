...
export default class MongoBinaryDownload {
  ...
  async getMongodPath(): Promise<string> {
    ...
    throw new Error(
      `Cannot find downloaded mongod binary by path ${mongodPath}`
    );
  }
  ...
  async checkMd5(mongoDBArchiveMd5: string, mongoDBArchive: string) {
    const signatureContent = fs
      .readFileSync(mongoDBArchiveMd5)
      .toString("UTF-8");
  ...
  async download(downloadUrl: string) {
    ...
    if (!filename) {
      throw new Error(
        `MongoBinaryDownload: missing filename for url ${downloadUrl}`
      );
    }
    ...
    this.debug(
      `Downloading${proxy ? ` via proxy ${proxy}` : ""}:`,
      downloadUrl
    );
  ...
  async extract(mongoDBArchive: string): Promise<string> {
    ...
    if (
      !this.locationExists(
        path.resolve(this.downloadDir, this.version, binaryName)
      )
    ) {
      throw new Error(
        `MongoBinaryDownload: missing mongod binary in ${mongoDBArchive}`
      );
    }
    return extractDir;
  }
